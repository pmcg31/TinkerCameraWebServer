...
static esp_err_t index_handler(httpd_req_t *req)
{
    ...
    {
        fileName = "/spiffs/index_ov3660.html";
    }
    ...
    if (err != ESP_OK)
    {
        ...
        case ESP_ERR_NOT_FOUND:
            ...
            break;
        case ESP_FAIL:
            Serial.println("ESP_FAIL (yeah, thanks...)");
            break;
        default:
    ...
    {
        size_t count = fread(index_handler_buffer, 1, IDX_BUFLEN, file);
        fclose(file);
        if (count != 0)
        {
            esp_vfs_spiffs_unregister(0);
            return httpd_resp_send(req, index_handler_buffer, count);
        }
        else
        {
            Serial.print("Empty read from file ");
